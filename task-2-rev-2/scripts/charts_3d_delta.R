...
{
len_lotss <- nrow(lotss_neightbour_results_nearest)
x<-1:len_lotss
...
{
  new_ra[lotss_num]<-(lotss_neightbour_results_nearest[lotss_num,"gaia_converted_ra"])
  new_dec[lotss_num]<-(lotss_neightbour_results_nearest[lotss_num,"gaia_converted_dec"])
}
}
...
scatterplot3d(x=x, y=new_ra, z=new_dec, 
              angle = 45, 
              grid = TRUE,
              main="Wykres 3d dla współrzędnych znalezionych sąsiadów 45deg",
              xlab = "Liczba porządkowa współrzędnej",
              ylab = "Skorygowana wartość rektascensji [deg]",
              zlab = "Skorygowana wartość deklinacji [deg]")
scatterplot3d(x=x, y=new_ra, z=new_dec, 
...
for (lotss_num in(1:len_lotss))
{
  delta_ra[lotss_num]<-abs(lotss_neightbour_results_nearest[lotss_num,"gaia_converted_ra"]-lotss_neightbour_results_nearest[lotss_num,"ra"])
  delta_dec[lotss_num]<-abs(lotss_neightbour_results_nearest[lotss_num,"gaia_converted_dec"]-lotss_neightbour_results_nearest[lotss_num,"dec"])
}
plot(delta_ra, x)
plot(delta_dec, x)
