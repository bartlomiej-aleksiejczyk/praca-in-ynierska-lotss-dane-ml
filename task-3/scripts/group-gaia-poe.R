...
{
  occurences[limit_index] <- (length(which(all_gaia_poe_over_7$parallax >= bottom_limit[limit_index] & all_gaia_poe_over_7$parallax < upper_limit[limit_index])))
}
gaia_poe_7_query <- cbind(bottom_limit,upper_limit,occurences)
